...
def _gen_mm_extra_hash_keys(request: Request, start_token_idx: int,
    ...
    # Note that we assume mm_features is sorted by offset.
    # We do not need to check all mm inputs if the start token index is out of
    # range. This usually happens in the late prefill phase and decoding phase.
    if mm_features[-1].mm_position.offset + mm_features[-1].mm_position.length < start_token_idx:
        return extra_keys, start_mm_idx
    ...
    while mm_features and curr_mm_idx < len(mm_features):
        assert mm_features[curr_mm_idx].identifier is not None
        offset = mm_features[curr_mm_idx].mm_position.offset
        length = mm_features[curr_mm_idx].mm_position.length
        if end_token_idx > offset:
            ...
            extra_keys.append(mm_features[curr_mm_idx].identifier)
